...
class HexKeyBoard(VirtualKeyBoard):
	def __init__(self, session, title="", **kwargs):
		...
		self.keys_list = [[["EXIT", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "BACKSPACE"], ["OK", "A", "B", "C", "D", "E", "F", "OK", "LEFT", "RIGHT", "ALL", "CLEAR"]]]
...
def keymenu(session, service=None, *args, **kwargs):
	...
	if not os.path.exists(SoftCamKey):
		...
		if caids and 0xe00 in caids:
			session.openWithCallback(boundFunction(setKeyCallback, session, SoftCamKey), HexKeyBoard, title=_("Please enter new key:"), text=findKeyPowerVU(session, SoftCamKey))
		elif caids and 0x2600 in caids:
			session.openWithCallback(boundFunction(setKeyCallback, session, SoftCamKey), HexKeyBoard, title=_("Please enter new key:"), text=findKeyBISS(session, SoftCamKey))
		elif caids and 0x604 in caids:
			session.openWithCallback(boundFunction(setKeyCallback, session, SoftCamKey), HexKeyBoard, title=_("Please enter new key:"), text=findKeyIRDETO(session, SoftCamKey))
		elif caids and 0x1010 in caids:
			session.openWithCallback(boundFunction(setKeyCallback, session, SoftCamKey), HexKeyBoard, title=_("Please enter new key:"), text=findKeyTandberg(session, SoftCamKey))
...
def setKeyCallback(session, SoftCamKey, key):
	...
	if key and len(key) == 14:
		...
		session.openWithCallback(boundFunction(setKeyCallback, session,SoftCamKey), HexKeyBoard, title=_("Invalid key, length is %d") % len(key), text=key.ljust(16,'*'))
