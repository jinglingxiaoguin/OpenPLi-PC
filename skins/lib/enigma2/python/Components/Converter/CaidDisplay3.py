...
#  05.10.2022 code optimizaton for Python3 mod by 1455
#
...
class CaidDisplay3(Poll, Converter, object):
		...
		if service:
			...
			if info:
				...
				if caids:
					...
					if ecm_info:
						emu_caid = ecm_info.get(b"caid", "").decode("utf-8", errors="b")
			...
			if info:
				if info.getInfoObject(iServiceInformation.sCAIDs):
					...
					if ecm_info:
						...
						hops = "HOPS: %s" % hops
						# ecm time
						ecm_time = ecm_info.get(b"ecm time", None).decode("utf-8", errors="b")
						if ecm_time:
							if "msec" in ecm_time:
