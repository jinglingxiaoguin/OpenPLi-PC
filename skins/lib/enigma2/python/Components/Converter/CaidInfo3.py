...
class CaidInfo3(Poll, Converter, object):
	CAID = 0
	CAID2 = 1
	CRYPT = 2
	CRYPT2 = 3
	...
	def getBoolean(self):
		...
		if caids:
			...
			if ecm_info:
				caid_a = ("%0.4X" % int(ecm_info.get(b"caid", ""),16))[:2]
				caid_b = ("%0.4X" % int(ecm_info.get(b"caid", ""),16))[2:]
				...
				if self.type == self.ROSS_C:
					...
					return False
				# oscam
				reader = ecm_info.get(b"reader", None)
				# cccam
		...
		if service:
			...
			if info:
				if info.getInfoObject(iServiceInformation.sCAIDs):
					if ecm_info:
				# caid
						caid = "%0.4X" % int(ecm_info.get(b"caid", ""),16)
				...
				# pid
						try:
							pid = "%0.4X" % int(ecm_info.get(b"pid", ""),16)
						except:
							pid = ""
				...
				# oscam
						try:
							prov = "%0.6X" % int(ecm_info.get(b"prov", ""),16)
						except:
							prov = ecm_info.get(b"prov", "")
						if self.type == self.PROV:
							return prov
						if ecm_info.get(b"ecm time", "").decode().find("msec") > -1:
							ecm_time = ecm_info.get(b"ecm time", "").decode()
						else:
							ecm_time = ecm_info.get(b"ecm time", "").decode().replace(".","").lstrip("0") + " msec"
						if self.type == self.DELAY:
							return ecm_time
				# protocol
						protocol = ecm_info.get(b"protocol", "").decode()
				# port
						port = ecm_info.get("port", "")
				# source
						source = ecm_info.get("source", "net") # hack!
				# server
						server = ecm_info.get("server", "")
				# hops
						hops = ecm_info.get(b"hops", "").decode()
				# system
						system = ecm_info.get("system", "")
				# provider
						provider = ecm_info.get(b"provider", "")
				# reader
						reader = ecm_info.get(b"reader", "").decode()
				...
				# ecm time
						...
						if self.type == self.ALL:
							if source == "emu":
								textvalue = "(Prov: %s, Caid: %s)" % (self.systemTxtCaids_a.get(caid[:2]), prov, caid)
				# new oscam ecm.info with port parametr
							elif reader != "" and source == "net" and port != "":
								textvalue = "Prov: %s, Caid: %s, ECM pid: %s, Reader: %s, %s - %s" % (prov, caid, pid, reader, protocol, ecm_time.replace("msec","ms"))
